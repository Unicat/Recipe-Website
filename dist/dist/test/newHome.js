...
exports.HeaderHtml = function (req, res, next) {
    ...
    var Cuisine = '';
    if (passedVariable === undefined && req.app.get('IDAccount') === undefined) {
        //Is user logged in?
        IDAccount = 0;
    } else if (req.app.get('IDAccount') === null) {
        //Is user logged in?
        IDAccount = 0;
    } else if (req.app.get('IDAccount') !== undefined && req.app.get('IDAccount') !== 'null') {
        IDAccount = req.app.get('IDAccount');
    } else {
        IDAccount = passedVariable; //if so Set IDAccount
        app.set('IDAccount', IDAccount);
    }
    //IDAccount = 1;
                    ...
                    if (error) {
                        ...
                        connection.query(AmountOfCommentsQuery1, function (error, results, fields) {
                            if (error) {
                                ...
                                rtnStr += ' recipes.</p>';
                                rtnStr += '</div>';
                                //thses queries find the suggestions for the user
                                            ...
                                            if (error) {
                                                ...
                                                for (var i = 0; i < count; i++) {
                                                    ...
                                                    rtnStr += multiline(function () {/*
                                                                                     ...
                                                                                     <div class="text-right"><a href="http://localhost:3000/recipetry/?IDRecipe=
                                                                                     */});
    ...
    function Add(IDAccount) {
        //code that needs to at the end of suggestion regardless of if logged in
        rtnStr += multiline(function () {/*
                                         </div>
                                         </div>
                                         </div>
